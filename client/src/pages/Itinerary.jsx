    ...
    <Scroll>
      ...
      <div className="px-6 mt-2 mb-4">
        <div className="flex items-center justify-between mb-4">
          ...
          <button className="flex items-center justify-center w-8 h-8 rounded-full bg-blue" onClick={() => setModal(true)}>
        ...
        <div className="absolute top-0 z-10 h-full w-96 bg-text/45">
          <div className="flex items-center justify-center w-full h-full">
            <div className="flex h-auto w-5/6 flex-col rounded bg-white p-3 gap-2.5">
              ...
              <h5 className="text-sm font-bold">Which destination would you like to add?</h5>
              <div className="flex flex-col gap-2">
                <select name="time" id="time" className="p-1 rounded shadow-md bg-yellow">
                  <option value="morning">Morning</option>
                  <option value="afternoon">Afternoon</option>
                  <option value="evening">Evening</option>
                  <option value="night">Night</option>
                </select>
                <input type="text" placeholder="Insert something..." value={query} onChange={handleSearch} className="px-2 py-1 shadow-md" />
              </div>
              {
                  ...
                  <div>
                    {
                      results.map((item) => {
                        return (
                          <div className="flex flex-grow gap-3 p-3 rounded shadow-md" key={item.ID} onClick={() => handleSelect(item)}>
                              <div className="w-16 h-16 rounded bg-blue"></div>
                              <div className="flex flex-col gap-1">
                                <h5 className="text-sm font-bold">{formatTitle(item.name)}</h5>
